...
describe ('QI_TS03 - Grid View Template - Single Sheet With Row, Col header, Formula Bar, Gridlines', function(client) {
	...
	before(function(client, done) {
		...
		logger.info("Launching the Leonardo Player URL: " + data.playerURL_Gridview_HBP)

		embedPlayer = client.page.embedPlayer();

    	client
    		.maximizeWindow()
    		.url(data.playerURL_Gridview_HBP)
	...
	it('TC02 - Check all error scenario', function(client) {
		...
		embedPlayer
			...
			.verify.elementCount(embedPlayer.elements.redColorCells.selector, 26)
	});

	it('TC03 - Validate Tooltip for cell B6', function(client) {

		embedPlayer
			.validateTooltipForCell('B6', 'Cell Text should be "5".')
	});
	...
	it('TC05 - Check My Work for few correct and few incorrect cell values - Text, Accounting, Percent and Decimal Values', function(client) {
		...
		embedPlayer.api
			...
			.enterText('2.00')
			.click(properties.get('checkMyWorkButton'))
			.verify.elementCount(embedPlayer.elements.greenColorCells.selector, 14)
	...
	it('TC07 - Check My Work after entering Formulas in cells (Incomplete)', function(client) {
		...
		client
			...
			.click(properties.get('checkMyWorkButton'))
			.verify.elementCount(embedPlayer.elements.greenColorCells.selector, 16)
